 ...
 */
long double quota_get_binding(const char* path, enum units unit, char* binding_quota)
{
  ...
  while(strcmp((char*) fuse_get_context()->private_data, current_path) != 0)
  {
    char value[256];
...
int quota_exceeded(const char* path)
{
  ...
  unsigned long quota = (unsigned long) quota_get_binding(path, BYTES, binding_quota);
